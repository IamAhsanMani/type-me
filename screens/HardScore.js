...
export default class EasyScore extends React.Component{
  ...
  getData = () => {
    fetch('https://ahsan2323.herokuapp.com/users/hardScore',
    {
    ...
    render() {
      console.log('lkkkk',Object.values(this.state.data).length)
      Object.values(this.state.data).map(v => {
        console.log('daaaaaaaa',v.length)
        v.map(d => {
    ...
    return(
        <View style={styles.container}>
          ...
          <View style={styles.ButtonView}>
              <Text style={{fontSize:19,fontWeight:'bold'}}>Hard Mode High Score</Text>
          { this.state.data ?
            Object.values(this.state.data).map(v => {
